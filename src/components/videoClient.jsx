...
export default function Button({ theme, click, children }) {
  ...
  const handlerPlay = () => {
    if (introClientVideo == false) {
      videoClientRef.current.play()
      setIntroClientVideo(!introClientVideo)
      setSoundClient(true)
    } else {
  ...
  return (
    <div className='fixed z-50'>
      <div className='z-30 absolute top-0 p-5 h-[50px] w-full'>
        {introClientVideo && <div className='flex'>
          <span className='relative flex  items-center justify-center z-50 bg-gray-800 w-[50px] text-[white] border-[2px] border-gray-50  text-center text-[16px] py-3 rounded-full' onClick={handlerSound}>
            ...
            {soundClient == false && <span className='absolute bg-gray-50 border-x-[1px] border-gray-800 transform rotate-45 w-[4px] h-full'></span>}
          </span>
        </div>}
      ...
      <div className={`video-player absolute w-screen lg:w-[300px] rounded-[20px]  flex items-center h-screen my-auto ${introClientVideo === true ? 'left-0 right-0 mx-auto' : 'left-[-200vw]'}`} >
