        ...
        <div className={`fixed w-[90%] max-w-[400px] h-[250px] right-[20px] pt-14 pb-[50px] px-2 flex flex-grow flex-col justify-end rounded-[10px] border-gray-200 shadow bg-[#F1E6E0] z-20 transition-all ${whatsapp ? 'bottom-[80px]' : 'bottom-[-100vw]'}`} >
            <div className='absolute top-0 left-0  w-full h-[50px] bg-[#00826A] rounded-t-[10px]'>
                ...
                <div className='absolute flex flex-col left-[50px] top-[12px] text-white text-[12px]'>
                    <span className='block text-[12px] m-0'>Precio Justo</span>
            ...
            <div className="mr-auto rounded-lg rounded-tl-none m-1 p-2 text-[12px] bg-white flex flex-col relative speech-bubble-left">
                <p className="text-[12px]">Hola q tal, dinos en q podemos <br /> ayudarte...</p>
                <p className="text-gray-600 text-[10px] text-right leading-none">{new Date().getHours()}:{new Date().getMinutes()}</p>
            </div>
            <div className='grid absolute w-full left-0 bottom-0 p-2 gap-2' style={{ gridTemplateColumns: 'auto 35px' }}>
                <textarea type="text" className=' w-full text-[12px] rounded-[20px] outline-none px-[10px] py-[10px]   resize-none' rows={1} onChange={onChangeWhatsapp} />
